...
const MAX_RUNNING_SPEED = 15 #30
const ACCEL = 0.7 #2
const DEACCEL = 5 #6

#jumping
var jump_height = 8 #15
var in_air = 0
...
func walk(delta):
	...
	var speed
	if Input.is_action_pressed("sprint(3D)"):
	...
	if has_contact and Input.is_action_just_pressed("jump(3D)"):
...
func fly(delta):
	...
	# check input and change direction
	if Input.is_action_pressed("move_forward(3D)"):
		direction -= aim.z
	if Input.is_action_pressed("move_backward(3D)"):
		direction += aim.z
	if Input.is_action_pressed("move_left(3D)"):
		direction -= aim.x
	if Input.is_action_pressed("move_right(3D)"):
